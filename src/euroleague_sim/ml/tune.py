...
def main(argv=None):
    ...
    parser.add_argument(
        "--study",
        default="all",
        help="Which study/studies to run: '1' (LogReg win), '2' (Ridge margin), "
             "'3' (CatBoost WFO), or 'all' (default). "
             "Examples: --study 3  |  --study 1,2  |  --study all",
    )
    args = parser.parse_args(argv or sys.argv[1:])

    # Parse --study into a set of ints; 'all' → {1, 2, 3}
    if args.study.strip().lower() == "all":
        run_studies: set[int] = {1, 2, 3}
    else:
        run_studies = {int(s.strip()) for s in args.study.split(",")}
